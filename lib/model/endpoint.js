...
Endpoint.prototype.getClusterNamesList = function () {
    var clusterList = this.getClusterList();
    if (!clusterList || clusterList instanceof Error) return [];
    var clusterNamesList = [];
    clusterList.forEach(function(cId) {
        clusterNamesList.push(zclId.cluster(cId));
    });
