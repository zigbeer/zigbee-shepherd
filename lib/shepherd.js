...
function ZShepherd(path, opts) {
    ...
    if (!_.isPlainObject(opts))
        throw new TypeError('opts should be an object if gieven.');
...
ZShepherd.prototype._findDevByAddr = function (addr) {
    ...
    if (!_.isString(addr) && !_.isNumber(addr))
        throw new TypeError('addr should be a number or a string.');
...
ZShepherd.prototype._findEndpoint = function (addr, epId) {
    ...
    if (!_.isNumber(epId))
        throw new TypeError('epId should be a number.');
