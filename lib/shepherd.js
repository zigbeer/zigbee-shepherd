...
ZShepherd.prototype.lqiScan = function (done) {
		...
		try {
			if(!err){
				for(var i=0;i<data.length;i++){
					...
					var emitRecursive = function(device){
						if(ieeeAddr != info.net.ieeeAddr){
							var endpoints = [];

							_.forEach(device.epList, function (epId) {
								var endpoint = device.getEndpoint(epId)
					...
					if(device){
						...
						device.joinTime = Math.floor(Date.now()/1000);
						self._devbox.set(info.net.ieeeAddr, device, function(){});
						emitRecursive(device);
					}else{
						//self._registerDev({},emitRecursive)
						//TODO: later on we could call the required ZDOs to figure out the ep's and other details
						return;				
					}
				}
			}
	...
	learn(false, [{ieeeAddr: info.net.ieeeAddr}]);
}
