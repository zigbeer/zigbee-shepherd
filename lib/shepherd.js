...
ZShepherd.prototype.start = function (app, callback) {
    ...
    controller.start(function (err) {
        if (err) {
            ...
            self.app = app;
            self.permitJoin('coord', 0, function (err) { if (err) console.log(err); });
            self.registerDev(controller.getCoord(), function (err, id) {
                if (err) {
                    console.log(err);
                }
            });
            ...
            loader.reload(self, function (err) {
                if (err) {
                    ...
                    self.permitJoin('coord', 0xff, function (err) { if (err) console.log(err); });
                    callback(null);
                }
            });
        }
...
ZShepherd.prototype.registerZApp = function (zApp, callback) {
    ...
    controller.registerEp(loEp).then(function (result) {
        ...
        coord.epList.push(loEp.getEpId());
    ...
    if (dev) {
        ...
        _.forEach(dev.endpoints, function (ep) {
            clustersReqs.push((function () {
                ...
                af.zclClustersReq(ep).then(function (clusters) {
        ...
        Q.all(clustersReqs).then(function() {
        ...
        this.registerDev(dev, function (err, id) {
            if (err) {
                dev = null;
                console.log(err);
            } else {
...
ZShepherd.prototype.attachZclMethods = function (ep) {
    ...
    if (ep.isLocal()) {
        ...
        ep.foundation = function (cId, cmd, zclData, cfg, callback) {
            return af.zclFoundation(ep, ep, cId, cmd, zclData, cfg, callback);
        };
        ep.functional = function (cId, cmd, zclData, cfg, callback) {
            return af.zclFunctional(ep, ep, cId, cmd, zclData, cfg, callback);
        };
    }
};
