...
function ZShepherd(path, opts) {
	...
	this._backupDbPath = opts.backupDbPath;
	...
	if (!this._backupDbPath) {    // use default
		this._backupDbPath = __dirname + '/database/dev.db.bak';
		// create default db folder if not there
		try {
			fs.statSync(__dirname + '/database');
		} catch (e) {
...
ZShepherd.prototype.generateMap = function(opts, callback){
	...
	var emitRecursive = function(device,ieeeAddr,nwkAddr){
		...
		queue.push(function(){
			try {
				counter ++;
				self.lqi(ieeeAddr, function(err, data){learn(err, data, ieeeAddr)});
			}catch(ex){
	...
	//Figure out the data in the current response
	learn = function(err,data, fromieeeAddr){
		if(!err){
			for(var i=0;i<data.length;i++){
				...
				if(typeof device !== "undefined"){
					...
					if(fromieeeAddr !== ieeeAddr){

						var oldlink = {};

						for(var link in links){
							if((links[link].from == fromieeeAddr || links[link].to == fromieeeAddr) &&
								(links[link].from == ieeeAddr || links[link].to == ieeeAddr)){
								oldlink = links[link];
							}
						}

						if(typeof oldlink.lqi !== "undefined"){
							oldlink.lqi = ( data[i].lqi + oldlink.lqi ) / 2;
						}else{
							links.push({
								from: fromieeeAddr,
								to: ieeeAddr,
		...
		if(counter == 0){
			...
			if(q){
				...
				else console.log('done', links);
			}
		}
	}
	learn(false, [{ieeeAddr: info.net.ieeeAddr, force: true}], info.net.ieeeAddr);
}
