...
ZShepherd.prototype.lqiScan = function (done) {
	...
	var emitRecursive = function(device,ieeeAddr,nwkaddr){
		if(ieeeAddr != info.net.ieeeAddr){
			...
			self.controller.simpleDescReq(nwkaddr, ieeeAddr).then(function (devInfo) {
				console.log(JSON.stringify(devinfo))
				return devInfo;
			}).fail(function () {
				return self.controller.simpleDescReq(nwkaddr, ieeeAddr);
			}).then(function (devInfo) {
				self.controller.emit('ZDO:devIncoming', devInfo);
			}).fail(function (err) {
				self.emit('error', 'Cannot get the Node Descriptor of the Device: ' + ieeeAddr + ' ('+err+')');
			}).done();
		}
