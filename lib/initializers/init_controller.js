...
init.setupCoord = function (controller, callback) {
    return controller.checkNvParams().then(function () {
        ...
        return init._registerDelegators(controller, nwkInfo);
    }).nodeify(callback);
};
...
init._bootCoordFromApp = function (controller) {
    return controller.querie.coordState().then(function (state) {
        ...
        // nwkInfo: { state, channel, panId, extPanId, ieeeAddr, nwkAddr }
...
init._registerDelegators = function (controller, nwkInfo) {
    ...
    return controller.simpleDescReq(0, nwkInfo.ieeeAddr).then(function (devInfo) {
        var registerDlgs = [];
