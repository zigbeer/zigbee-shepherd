    ...
    return controller.querie.coord().then(function (coordInfo) {
        ...
        if (!isCoordRunning) {
            ...
            _.forEach(dlgInfos, function (dlgInfo) {
                var dlgEp = new Coordpoint(coord, { profId: dlgInfo.profId, epId: dlgInfo.epId, devId: 0x0005, inClusterList: [], outClusterList: [] });
        ...
        _.forEach(eps, function (ep) {
            if (ep.isDelegator()) {
                registerDlgs.push(function () {
                    return controller.registerEp(ep).delay(10).then(function () {
                        debug('Register delegator, epId: %s, profId: %s ', ep.getEpId(), ep.getProfId());
                    });
                });
            }
        });
