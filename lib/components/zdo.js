...
var Q = require('q'),
    ...
    ADDRESS_MODE = ZDEF.AF.addressMode;
...
Zdo.prototype._genericRequest = function (apiName, valObj, callback) {
    ...
    if (areqEvtKey)
        controller._areq.register(areqEvtKey, deferred, function (payload) {
            controller._areq.resolve(areqEvtKey, payload);
        });

    this._sendZdoRequestViaZnp(apiName, valObj, function (err, rsp) {
        if (err)
            controller._areq.reject(areqEvtKey, err);
    });
