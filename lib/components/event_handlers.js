...
handlers.devIncoming = function (devInfo) {
    ...
    Q.fcall(function () {
		...
		try {
			...
			var basicEndpoint = dev.getEndpoint(basicEndpoint);
			if (!basicEndpoint || basicEndpoint instanceof Error) return dev;

			// Get manufName, modelId and powerSource information
			return self.af.zclFoundation(dev.getEndpoint(basicEndpoint), dev.getEndpoint(basicEndpoint), 0, 'read', [{ attrId: 4 }, { attrId: 5 }, { attrId: 7 }])
	    ...
	    interviewEvents.on('ind:interview:cluster', function(status) {
		    self.emit('ind:interview', dev.ieeeAddr, {
			    endpoint:{
				    total: numberOfEndpoints,
				    current: i,
				    cluster: status
			    }
		    });
