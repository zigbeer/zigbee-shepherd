...
function dispatchIncomingMsg(type, msg) {
    ...
    if (msg.hasOwnProperty('endpoint')) {                                               // dataConfirm, reflectError
        ...
        if (targetEp && targetEp.isDelegator())
    ...
    switch (type) {
        ...
        case 'zclIncomingMsg':
            ...
            if (frameType === 0) {          // foundation
                ...
                if (msg.zclMsg.cmdId === 'report') {
                    var cIdString = zclId.cluster(msg.clusterid),
                        data = {};

                    cIdString = cIdString ? cIdString.key : msg.clusterid;

                    _.forEach(msg.zclMsg.payload, function (reportRec) {  // { attrId: 0, dataType: 33, attrData: 5217 }
                        var attrIdString = zclId.attr(cIdString, reportRec.attrId);
                        attrIdString = attrIdString ? attrIdString.key : reportRec.attrId;
                        data[attrIdString] = reportRec.attrData;
                    });

                    af.controller.getShepherd().emit('ind:changed', targetEp, { cid: cIdString, data: data });
                }
