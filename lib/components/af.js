...
af.zclFoundation = function (srcEp, dstEp, cId, cmd, zclData, cfg, callback) {
    ...
    var deferred = Q.defer(),
        areq = af.areq,
        dir = (srcEp === dstEp) ? 1 : 0,    // 1 [0]: client-to-server, 0 [1]: server-to-client [TODO] wrong direction!!!
        manufCode = 0,
        frameCntl,
        seqNum,
        zclBuffer,
...
// clusters: {
//    genBasic: { dir: 1, attrs: { x1: { value: 0 }, x2: { value: 3}, ... } },   // dir => 0: 'unknown', 1: 'in', 2: 'out'
//    fooClstr: { dir: 1, attrs: { x1: { value: 0 }, x2: { value: 3}, ... } },
//    ...
// }
    ...
    var clusterList = dstEp.getClusterList(),       // [ 1, 2, 3, 4, 5 ]
        inClusterList = dstEp.getInClusterList(),
        outClusterList = dstEp.getOutClusterList(),
        clusterAttrsReqs = [];

    _.forEach(clusterList, function (cId) {         // cId is a number
        var cIdString = zclId.cluster(cId);
        cIdString = cIdString ? cIdString.key : cId;
        clusters[cIdString] = { dir: 0, attrs: null };

        // push the request promise to clusterAttrsReqs
        clusterAttrsReqs.push(
            af.zclClusterAttrsReq(dstEp, cId).then(function (attrs) {
                clusters[cIdString].attrs = attrs;
                // ensure cluster direction
