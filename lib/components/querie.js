...
querie.cluster = function (dstEp, cId, callback) {
    // [TODO] foundation method
    var deferred = Q.defer(),
        cInfo = {
            cId: cId,
            dir: null,
    ...
    controller.zclDiscover(dstEp, cId, 0, 240).then(function (rsp) {
        // rsp: { discComplete, attrInfos: [ { attrId, dataType }, ... ] }
        rsp.attrInfos.forEach(function (attrInfo) {
            cInfo.attrList.push(attrInfo.attrId);
        });
...
querie.attributes = function (dstEp, cId, attrIdArray, callback) {
    // [TODO] foundation method
    var deferred = Q.defer(),
        attrs = {};

    controller.zclRead(dstEp, cId, attrIdArray).then(function (rsp) {
        // rsp: [ { attrId, status[, dataType, attrData] }, ... ]
        rsp.forEach(function (attr, idx) {
            var attrValue = null;

            if (attr.status === 0 || attr.status === 'SUCCESS')
                attrValue = attr.attrData;
            // [TODO]  acl: null
            attrs[attr.attrId] = { acl: null, value: attrValue };
        });
...
querie.clusterWithAttributes = function (dstEp, cId, callback) {
    var deferred = Q.defer(),
        fullCluster;

    querie.cluster(dstEp, cId).then(function (cInfo) {
        fullCluster = cInfo;
        return fullCluster;
    }).then(function () {
        return querie.attributes(dstEp, cId, fullCluster.attrList);
    }).then(function (attrs) {
        fullCluster.attributes = attrs;
        deferred.resolve(fullCluster);
    }).fail(function (err) {
        deferred.reject(err);
    }).done();
...
querie.routingTable = function (dstaddr, callback) {
    return controller.request('ZDO', 'mgmtRtgReq', { dstaddr: dstaddr, startindex: 0 });
};

querie.neighborTable = function (dstaddr, callback) {
    return controller.request('ZDO', 'mgmtLqiReq', { dstaddr: dstaddr, startindex: 0 });
};
...
querie.setBindingEntry = function (bindMode, remoteEp, localEp, cId, callback) {
    ...
    if (bindMode === 0)       // bimd
        controller.request('ZDO', 'bindReq', bindParams);
    else if (bindMode === 1)  // unbind
        controller.request('ZDO', 'unbindReq', bindParams);
};
