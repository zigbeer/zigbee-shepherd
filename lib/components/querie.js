...
querie.device = function (ieeeAddr, nwkAddr, callback) {
    ...
    if (!_.isString(ieeeAddr))
        throw new TypeError('ieeeAddr should be a string.');

    if (!_.isNumber(nwkAddr) || _.isNaN(nwkAddr))
        throw new TypeError('nwkAddr should be a number.');
...
querie.setBindingEntry = function (bindMode, srcEp, cId, dstEpOrGrpId, callback) {
    ...
    if (_.isUndefined(cIdItem))
        throw new TypeError('Invalid cluster id: ' + cId + '.');
    ...
    bindParams = {
        ...
        addr_short_long: dstEp ? dstEp.getDevice().getIeeeAddr() : toLongAddrString(grpId),
            ...
            if (err)
                ...
                switch (param) {
                    ...
                    case 'PARENT_SHORT_ADDR':
                        ...
                        break;
                    case 'PAN_ID':
                        deferred.resolve(zutils.toHexString(rsp.value.readUInt16LE(0), 'uint16'));
                        break;
                }
            }
        });
