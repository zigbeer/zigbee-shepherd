...
//  - incomingMsgExt,   { groupid, clusterid, srcaddrmode, srcaddr, srcendpoint, srcpanid, dstendpoint, wasbroadcast, linkquality, securityuse, timestamp, transseqnumber, len, data }
    ...
    switch (msg.ind) {
        case 'dataConfirm':
            ...
            // find if listner there? if not, emit unhandled   >>> epSelf.emit('ZCL_CNF:UNHANDLED', cnfMsg);
        ...
        case 'incomingMsg':
            ...
            // zcl parsing
            bridge._zclIndicationEventBridge(controller, msg);
            break;
        case 'incomingMsgExt':
...
bridge._zclIndicationEventBridge = function (controller, msg) {
    ...
    zclHeader = zcl.header(zclPacket);
