// [TODO] bridge methods for all subsystems are not completed
var zcl = require('./zcl');
var bridge = {
...
bridge.generateEvent = function (controller, msg) {
    var event;
...
// msghub.emit('AF:INCOMING_MSG:' + (msgobj.msg.srcaddr) + ':' + (msgobj.msg.clusterid) + ':' + (msgobj.msg.dstendpoint), msgobj.msg);
// remote ep: 'AF:INCOMING_MSG:' + targetDev.ieeeAdd + ':' + targetEpId
// local ep: 'AF:INCOMING_MSG:' + coordDev.ieeeAddr ':' + msgobj.msg.dstendpoint
bridge.generateAfEvent = function (controller, msg) {
    var event,
        targetDev,
        targetEp,
        data = msg.data,
        shepherd = controller._shepherd,
    ...
    if (msg.ind === 'incomingMsg') {
        event = 'AF:incomingMsg:' + data.srcaddr.toString() + ':' + data.clusterid.toString() + ':' + data.dstendpoint.toString();
        targetDev = shepherd.findDev({ nwk: data.srcaddr });
        event = 'AF:incomingMsg:' + targetDev.getIeeeAddr() + ':' + data.srcendpoint.toString();
        event = 'AF:incomingMsg:' + coord.getIeeeAddr() + ':' + data.dstendpoint.toString();
        // zcl event?
    } else if (msg.ind === 'dataConfirm') {
        event = 'AF:dataConfirm:' + data.endpoint.toString();
        event = 'AF:dataConfirm:' + data.endpoint.toString() + ':' + data.transid.toString();
    }
...
bridge.generateZdoEvent = function (controller, msg) {
    var event;
...
bridge.hasAreq = function (subsys, cmdId) {
