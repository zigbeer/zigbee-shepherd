...
var clusters = {
    ...
    genIdentify: {
        dir: 0,
        attrs: {
            identifyTime: { acl: READ | WRITE, value: 0 }
        }
    ...
    ssIasWd: {
        dir: 0,
        attrs: {
            maxDuration: { acl: READ | WRITE, value: 240 }
        }
    ...
    ssIasZone: {
        dir: 0,
        attrs: {
            zoneState: { acl: READ, value: 0 },
            zoneType: { acl: READ, value: 0 },
            zoneStatus: { dacl: READ, value: 0 },
            iasCieAddr: { acl: READ | WRITE, value: '0x124356' },
            zoneId: { acl: READ, value: 0 }
        }
...
function ZApp(shepherd, ids, clusters, discCmds) {
    ...
    if (!coord.findEndpoint(ids.epId)) {
        this.endpoint = new Coordpoint(coord, this.epInfo);
        this.endpoint.createClusters(clusters);          // add cId to inCList and outCList
        this.endpoint.createDiscoverableCmds(discCmds);
    } else {
        throw new Error('Endpoint already exists. Choose another enpoint id.');
    }
