...
Controller.prototype.afRegister = function (ep, callback) {
    return afSender.afRegister(this, ep, callback);
};

Controller.prototype.afDelete = function (epId, callback) {
    return afSender.afDelete(this, epId, callback);
};
...
Controller.prototype.zclCommand = function (localEp, dstEp, cId, cmd, valObj, callback)  {
    // .frame(frameCntl, manufCode, seqNum, cmd, zclPayload[, clusterId])
    var deferred = Q.defer(),
        dev = dstEp.getDev(),
        frameCntl = { frameType: 1, manufSpec: 0, direction: 0, disDefaultRsp: 0 },
    ...
    this.afDataSendByLocalEp(localEp, dstEp, cId, zclPacket, {}).fail(function (err) {
        deferred.reject(err);
    }).done();
...
Controller.prototype._initCoordAfterConnected = function (nwkInfo) {
    ...
    this.queryCoordInfo().then(function (coordInfo) {   // coordInfo = { type, ieeeAddr, nwkAddr, manufId, epList }
        if (!isCoordRunning) {
            ...
            self._delegators = null;                    // clear all delegators
            ...
            self._delegators = [ dlgIPM, dlgHA, dlgCBA, dlgTA, dlgPHHC, dlgSE ];
        }
