...
Controller.prototype.setNetInfo = function (nwkInfo) {
    ...
    proving.object(nwkInfo, 'nwkInfo should be an object');
        ...
        steps = [
         ...
         // function () { return self.request('ZDO', 'startupFromApp', { startdelay: 0 }).delay(10); },
            function () { return self.request('SYS', 'osalNvItemInit', nvParams.znpCfgItem).delay(10).fail(function (err) {
                if (err.message === 'rsp error: 9') {    // Success, item created and initialized
                    return;
                } else {
                    deferred.reject(err);
                    return deferred.promise.nodeify(callback);
...
Controller.prototype.request = function (subsys, cmdId, valObj, callback) {
    ...
    rspHdlr = function (err, rsp) {
        if (subsys !== 'ZDO' && subsys !== 5) {
            if (rsp && rsp.hasOwnProperty('status'))
                debug.response('%s <-- %s, status: %d', subsys, cmdId, rsp.status);
            else
                debug.response('%s <-- %s', subsys, cmdId);
        }
    ...
    if ((subsys === 'AF' || subsys === 4) && valObj.hasOwnProperty('transid'))
        debug.request('%s --> %s, transId: %d', subsys, cmdId, valObj.transid);
    else
        debug.request('%s --> %s', subsys, cmdId);
...
Controller.prototype.permitJoin = function (joinTime, joinType, callback) {
    ...
    proving.stringOrNumber(joinType, 'joinType should be a number or a string.');
    proving.number(joinTime, 'joinTime should be a number.');
...
Controller.prototype.deregisterEp = function (loEp, callback) {
    ...
    if (!_.includes(coordEps, loEp))
        deferred.reject(new Error('Endpoint not maintained by Coordinator, cannot be removed.'));
    else
...
Controller.prototype.reRegisterEp = function (loEp, callback) {
    ...
    if (!_.includes(this.getCoord().endpoints, loEp))
        deferred.reject(new Error('Endpoint not maintained by Coordinator, cannot be re-register.'));
    else
...
Controller.prototype.remove = function (dev, cfg, callback) {
    ...
    if (cfg.hasOwnProperty('reJoin'))
        proving.boolean(cfg.reJoin, 'cfg.reJoin should be a boolean.');

    if (cfg.hasOwnProperty('rmChildren'))
        proving.boolean(cfg.rmChildren, 'cfg.rmChildren should be a boolean.');
