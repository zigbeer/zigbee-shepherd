...
Controller.prototype._checkOnline = function (dev, callback) {
    ...
    this.request('ZDO', 'ieeeAddrReq', { shortaddr: dev.getNwkAddr(), reqtype: 0 , startindex: 0 }).timeout(3000).then(function (rsp) {
        // rsp: { status, ieeeaddr, nwkaddr, startindex, numassocdev, assocdevlist }

        if (dev.getIeeeAddr() !== rsp.ieeeaddr)
            deferred.resolve();

        self.emit('ZDO:endDeviceAnnceInd', { srcaddr: rsp.nwkaddr, nwkaddr: rsp.nwkaddr, ieeeaddr: rsp.ieeeaddr, capabilities: {} });
        deferred.resolve();
    }).fail(function (err) {
        deferred.resolve();
    }).done();
