...
function Controller(cfg) {
    ...
    zdo.sendRequest = znp.zdoRequest;

    this.request = function (subsys, cmdId, valObj, callback) {
        if (subsys.toUpperCase() === 'ZDO' || subsys === 5)
            return zdo.request(cmdId, valObj, callback);
        else
            return znp.request(subsys, cmdId, valObj, callback);
    };
}
