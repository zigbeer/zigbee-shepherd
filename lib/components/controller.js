...
function Controller(shepherd, cfg) {    // cfg is serial port config
    ...
    this._coord = new Coordinator({ type: 1, ieeeAddr: '', nwkAddr: 0, manufId: null, epList: [] });    // [TODO] move to init()
    ...
    this._znp.on('ready', function () {
        self.queryCoordInfo().then(function (coordInfo) {
            self._coord = new Coordinator(coordInfo);
            self.delegators.push(new Coordpoint(self._coord, { profId: 0x0101, epId: 1, devId: 0x0005, inCList: [], outCList: [] }, true)); // 'IPM': 0x0101, Industrial Plant Monitoring
            self.delegators.push(new Coordpoint(self._coord, { profId: 0x0104, epId: 2, devId: 0x0005, inCList: [], outCList: [] }, true)); // 'HA': 0x0104, Home Automation
            self.delegators.push(new Coordpoint(self._coord, { profId: 0x0105, epId: 3, devId: 0x0005, inCList: [], outCList: [] }, true)); // 'CBA': 0x0105, Commercial Building Automation
            self.delegators.push(new Coordpoint(self._coord, { profId: 0x0107, epId: 4, devId: 0x0005, inCList: [], outCList: [] }, true)); // 'TA': 0x0107, Telecom Applications
            self.delegators.push(new Coordpoint(self._coord, { profId: 0x0108, epId: 5, devId: 0x0005, inCList: [], outCList: [] }, true)); // 'PHHC': 0x0108, Personal Home & Hospital Care
            self.delegators.push(new Coordpoint(self._coord, { profId: 0x0109, epId: 6, devId: 0x0005, inCList: [], outCList: [] }, true)); // 'SE': 0x0109, Smart Energy 'AMI': 0x0109, Advanced Metering Initiative, Smart Energy

            return self.delegators;
        }).then(function (dlgs) {
            var registerResults = [];

            dlgs.forEach(function (dlgEp) {
                registerResults.push(self._coord.registerEndpoint(dlgEp));
            });
...
Controller.prototype._initCoord = function () {
    var self;

    return this.queryCoordState().then(function (state) {
        if (state === 'ZB_COORD' || 0x09)
            return self.queryNwkInfo(); // coord has started
        else
            return self._initCoordFromApp();
    }); // return nwkInfo
};

Controller.prototype._initCoordFromApp = function () {
    var self = this,
        waitBootTime = 3000;
...
Controller.prototype.setNwkInfo = function (key, val, callback) {
};
